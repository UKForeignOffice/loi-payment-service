...
module.exports = {
    ...
    addDateFields: function(formFields) {
        ...
        var shipBeforeDateValue = moment().add(1, 'd').format();
        var sessionValidityValue = moment().add(configSmartPay.configs.sessionValidity, 'm').format();
    ...
    calculateMerchantSignature: function(formFields) {
        ...
        var hash = crypto.createHmac('sha256', new Buffer.from(hmacKey, 'hex')).update(signingString, 'utf-8').digest('base64');
    ...
    decodeReturnedMerchantSignature: function(queryString) {
        ...
        // calculate HMAC string and encode in base64
        var hash = crypto.createHmac('sha256', new Buffer.from(hmacKey, 'hex')).update(queryStringValues, 'utf-8').digest('base64');
    ...
    additionalPaymentsAddBaseData: function (formFields, cost, email) {
        ...
        formFields.merchantReference = moment().unix();
        formFields.orderData = 'You are making an additional payment of £' + cost + ' to the Get Document Legalised Service';
        formFields.paymentAmount = Math.round(cost * 1e2);
        formFields.merchantReturnData = formFields.merchantReference;
        formFields.allowedMethods = '';
        formFields.shopperEmail = email;
        return formFields;
    },
    ...
    additionalPaymentsCalculateMerchantSignature: function(formFields) {
        ...
        // calculate HMAC string and encode in base64
        return crypto.createHmac('sha256', new Buffer.from(hmacKey, 'hex')).update(signingString, 'utf-8').digest('base64');
    },
