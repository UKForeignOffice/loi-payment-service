 ...
 */
export const sessionTtlMiddleware = (configGovPay) => {
  return (req, res, next) => {
    ...
    if (req.session?.cookie && !Number.isNaN(configuredSessionTtl) && configuredSessionTtl > 0) {
      const sessionTtl = parseInt(req.session.cookie.originalMaxAge, 10)

      if (Number.isNaN(sessionTtl) || sessionTtl < configuredSessionTtl) {
    ...
    if (req.cookies.LoggedIn && !Number.isNaN(configuredSessionTtl) && configuredSessionTtl > 0) {
      const sessionTtl = req.session?.cookie && parseInt(req.session.cookie.originalMaxAge, 10)
      const loggedInCookieMaxAge = !Number.isNaN(sessionTtl) && sessionTtl > 0 ? sessionTtl : configuredSessionTtl
